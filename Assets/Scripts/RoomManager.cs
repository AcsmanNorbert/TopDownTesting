...
public class RoomManager : MonoBehaviour
{
    ...
    private void SpawnEnemies()
    {
        ...
        List<Vector3> spawnablePoints = new List<Vector3>();
        foreach (GameObject point in spawnPoints)
        {
            ...
            spawnablePoints.Add(point.transform.position);
        }
        ...
        var furthestSpawnpoints = spawnablePoints.OrderByDescending(item => Vector3.Distance(playerRef.transform.position, item)).Take(spawnRate);
        
        foreach (Vector3 point in furthestSpawnpoints)
        {
            ...
            for (int i = 0; i < randomCard.count; i++)
            {
                ...
                if (randomCard.count > 1)
                {
                    ...
                    spawnBounds.center = spawnPosition;
